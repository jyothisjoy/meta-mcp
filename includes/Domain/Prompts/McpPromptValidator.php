...
class McpPromptValidator {
	...
	public static function validate_prompt_data( array $prompt_data, string $context = '' ) {
		...
		if ( ! empty( $validation_errors ) ) {
			...
			/* translators: %s: comma-separated list of validation errors */
				__( 'Prompt validation failed: %s', 'meta-mcp' ),
	...
	public static function validate_prompt_dto( PromptDto $prompt ) {
		...
		if ( ! McpValidator::validate_name( $prompt->getName() ) ) {
			$errors[] = __( 'Prompt name must be 1-128 characters and contain only [A-Za-z0-9_.-]', 'meta-mcp' );
		}
	...
	public static function get_validation_errors( array $prompt_data ): array {
		...
		if ( empty( $prompt_data['name'] ) || ! is_string( $prompt_data['name'] ) || ! McpValidator::validate_name( $prompt_data['name'] ) ) {
			$errors[] = __( 'Prompt name is required and must be 1-128 characters and contain only [A-Za-z0-9_.-]', 'meta-mcp' );
		}

		// Check optional fields if present
		if ( isset( $prompt_data['title'] ) && ! is_string( $prompt_data['title'] ) ) {
			$errors[] = __( 'Prompt title must be a string if provided', 'meta-mcp' );
		}

		if ( isset( $prompt_data['description'] ) && ! is_string( $prompt_data['description'] ) ) {
			$errors[] = __( 'Prompt description must be a string if provided', 'meta-mcp' );
		}
	...
	private static function get_arguments_validation_errors( $arguments ): array {
		...
		if ( ! is_array( $arguments ) ) {
			return array( __( 'Prompt arguments must be an array if provided', 'meta-mcp' ) );
		}
		...
		foreach ( $arguments as $index => $argument ) {
			if ( ! is_array( $argument ) ) {
				...
				/* translators: %d: argument index */
					__( 'Prompt argument at index %d must be an object', 'meta-mcp' ),
					$index
				);
				continue;
			}
			...
			if ( empty( $argument['name'] ) || ! is_string( $argument['name'] ) ) {
				$errors[] = sprintf(
				/* translators: %d: argument index */
					__( 'Prompt argument at index %d must have a non-empty name string', 'meta-mcp' ),
					$index
				);
				continue;
			}
			...
			if ( ! McpValidator::validate_name( $argument['name'] ) ) {
				...
				/* translators: %s: argument name */
					__( 'Prompt argument \'%s\' name must only contain letters, numbers, hyphens (-), underscores (_), and dots (.), and be 128 characters or less', 'meta-mcp' ),
					$argument['name']
				);
			}
			...
			if ( isset( $argument['description'] ) && ! is_string( $argument['description'] ) ) {
				$errors[] = sprintf(
				/* translators: %s: argument name */
					__( 'Prompt argument \'%s\' description must be a string if provided', 'meta-mcp' ),
					$argument['name']
				);
			}
			...
			$errors[] = sprintf(
			/* translators: %s: argument name */
				__( 'Prompt argument \'%s\' required field must be a boolean if provided', 'meta-mcp' ),
				$argument['name']
			);
		}
		...
		foreach ( $messages as $index => $message ) {
			if ( ! is_array( $message ) ) {
				...
				/* translators: %d: message index */
					__( 'Message at index %d must be an object', 'meta-mcp' ),
					$index
				);
				continue;
			}
			...
			if ( empty( $message['role'] ) || ! is_string( $message['role'] ) ) {
				$errors[] = sprintf(
				/* translators: %d: message index */
					__( 'Message at index %d must have a role field', 'meta-mcp' ),
					$index
				);
				continue;
			}
			...
			if ( ! in_array( $message['role'], array( 'user', 'assistant' ), true ) ) {
				$errors[] = sprintf(
				/* translators: %d: message index */
					__( 'Message at index %d role must be either \'user\' or \'assistant\'', 'meta-mcp' ),
					$index
				);
			}
			...
			if ( empty( $message['content'] ) || ! is_array( $message['content'] ) ) {
				$errors[] = sprintf(
				/* translators: %d: message index */
					__( 'Message at index %d must have a content object', 'meta-mcp' ),
					$index
				);
				continue;
			}
			...
			foreach ( $icons_result['errors'] as $error_group ) {
				foreach ( $error_group['errors'] as $error ) {
					...
					/* translators: 1: icon index, 2: error message */
						__( 'Icon at index %1$d: %2$s', 'meta-mcp' ),
						$error_group['index'],
						$error
					);
				}
	...
	private static function get_content_validation_errors( array $content, int $message_index ): array {
		...
		if ( empty( $content['type'] ) || ! is_string( $content['type'] ) ) {
			return array(
				sprintf(
				/* translators: %d: message index */
					__( 'Message %d content must have a type field', 'meta-mcp' ),
					$message_index
				),
			);
		}
		...
		switch ( $type ) {
			case 'text':
				if ( ! isset( $content['text'] ) || ! is_string( $content['text'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d text content must have a text field', 'meta-mcp' ),
						$message_index
					);
				}
				break;
			...
			case 'image':
				if ( empty( $content['data'] ) || ! is_string( $content['data'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d image content must have a data field with base64-encoded image', 'meta-mcp' ),
						$message_index
					);
				} elseif ( ! McpValidator::validate_base64( $content['data'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d image content data must be valid base64', 'meta-mcp' ),
						$message_index
					);
				}
				...
				if ( empty( $content['mimeType'] ) || ! is_string( $content['mimeType'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d image content must have a mimeType field', 'meta-mcp' ),
						$message_index
					);
				}
				break;
			...
			case 'audio':
				if ( empty( $content['data'] ) || ! is_string( $content['data'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d audio content must have a data field with base64-encoded audio', 'meta-mcp' ),
						$message_index
					);
				} elseif ( ! McpValidator::validate_base64( $content['data'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d audio content data must be valid base64', 'meta-mcp' ),
						$message_index
					);
				}
				...
				if ( empty( $content['mimeType'] ) || ! is_string( $content['mimeType'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d audio content must have a mimeType field', 'meta-mcp' ),
						$message_index
					);
				}
				break;
			...
			case 'resource_link':
				...
				if ( empty( $content['name'] ) || ! is_string( $content['name'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d resource_link content must have a name field', 'meta-mcp' ),
						$message_index
					);
				}
				...
				if ( empty( $content['uri'] ) || ! is_string( $content['uri'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d resource_link content must have a uri field', 'meta-mcp' ),
						$message_index
					);
				} elseif ( ! McpValidator::validate_resource_uri( $content['uri'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d resource_link content uri must be a valid URI format', 'meta-mcp' ),
						$message_index
					);
				}
				...
				if ( isset( $content['mimeType'] ) && ! is_string( $content['mimeType'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d resource_link content mimeType must be a string if provided', 'meta-mcp' ),
						$message_index
					);
				}
				...
				if ( isset( $content['size'] ) && ! is_int( $content['size'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d resource_link content size must be an integer if provided', 'meta-mcp' ),
						$message_index
					);
				}
				...
				if ( isset( $content['title'] ) && ! is_string( $content['title'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d resource_link content title must be a string if provided', 'meta-mcp' ),
						$message_index
					);
				}
				...
				if ( isset( $content['description'] ) && ! is_string( $content['description'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d resource_link content description must be a string if provided', 'meta-mcp' ),
						$message_index
					);
				}
				...
				if ( isset( $content['icons'] ) ) {
					if ( ! is_array( $content['icons'] ) ) {
						$errors[] = sprintf(
						/* translators: %d: message index */
							__( 'Message %d resource_link content icons must be an array if provided', 'meta-mcp' ),
							$message_index
						);
					} else {
			...
			case 'resource':
				if ( empty( $content['resource'] ) || ! is_array( $content['resource'] ) ) {
					$errors[] = sprintf(
					/* translators: %d: message index */
						__( 'Message %d resource content must have a resource object', 'meta-mcp' ),
						$message_index
					);
				} else {
					...
					foreach ( $resource_errors as $resource_error ) {
						...
						/* translators: %1$d: message index, %2$s: resource error */
							__( 'Message %1$d embedded resource: %2$s', 'meta-mcp' ),
							$message_index,
							$resource_error
						);
					}
			...
			default:
				...
				/* translators: %1$d: message index, %2$s: content type */
					__( 'Message %1$d content type \'%2$s\' is not supported. Must be \'text\', \'image\', \'audio\', \'resource\', or \'resource_link\'', 'meta-mcp' ),
					$message_index,
					$type
				);
				break;
		...
		if ( isset( $content['annotations'] ) ) {
			if ( ! is_array( $content['annotations'] ) ) {
				$errors[] = sprintf(
				/* translators: %d: message index */
					__( 'Message %d content annotations must be an array if provided', 'meta-mcp' ),
					$message_index
				);
			} else {
