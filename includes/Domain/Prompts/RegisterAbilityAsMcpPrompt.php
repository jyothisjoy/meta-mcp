		...
		foreach ( $explicit_arguments as $index => $arg ) {
			if ( ! is_array( $arg ) ) {
				return new WP_Error(
					...
					/* translators: 1: argument index, 2: ability name */
						__( 'Argument at index %1$d must be an array for ability "%2$s".', 'meta-mcp' ),
						$index,
			...
			if ( ! isset( $arg['name'] ) || ! is_string( $arg['name'] ) || '' === trim( $arg['name'] ) ) {
				return new WP_Error(
					...
					sprintf(
					/* translators: 1: argument index, 2: ability name */
						__( 'Argument at index %1$d is missing required "name" field for ability "%2$s".', 'meta-mcp' ),
						$index,
						$this->ability->get_name()
					)
				);
		...
		if ( ! is_string( $filtered_name ) || ! McpValidator::validate_name( $filtered_name ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Filter returned invalid MCP prompt name: %s', 'meta-mcp' ),
		...
		try {
			...
			return new WP_Error(
				...
				/* translators: %s: error message */
					__( 'Failed to create Prompt DTO for ability %1$s: %2$s', 'meta-mcp' ),
					$ability->get_name(),
					$e->getMessage()
				),
				array( 'exception' => $e )
