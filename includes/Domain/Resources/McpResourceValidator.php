...
class McpResourceValidator {
	...
	public static function validate_resource_data( array $resource_data, string $context = '' ) {
		...
		if ( ! empty( $validation_errors ) ) {
			...
			/* translators: %s: comma-separated list of validation errors */
				__( 'Resource validation failed: %s', 'meta-mcp' ),
	...
	public static function validate_resource_dto( ResourceDto $resource_dto ) {
		...
		if ( ! McpValidator::validate_resource_uri( $resource_dto->getUri() ) ) {
			$errors[] = __( 'Resource URI must be a valid URI string', 'meta-mcp' );
		}
	...
	public static function get_validation_errors( array $resource_data ): array {
		...
		if ( empty( $resource_data['uri'] ) || ! is_string( $resource_data['uri'] ) ) {
			$errors[] = __( 'Resource URI is required and must be a non-empty string', 'meta-mcp' );
		} elseif ( ! McpValidator::validate_resource_uri( $resource_data['uri'] ) ) {
			$errors[] = __( 'Resource URI must be a valid URI format', 'meta-mcp' );
		}
		...
		if ( ! $has_text && ! $has_blob ) {
			$errors[] = __( 'Resource contents must include at least one of: text (string) or blob (base64 string)', 'meta-mcp' );
		}

		if ( $has_text_key && ! is_string( $resource_data['text'] ) ) {
			$errors[] = __( 'Resource text content must be a string when provided', 'meta-mcp' );
		}

		if ( $has_blob_key && ! is_string( $resource_data['blob'] ) ) {
			$errors[] = __( 'Resource blob content must be a string when provided', 'meta-mcp' );
		}

		// Validate blob content if present and typed.
		if ( $has_blob && ! McpValidator::validate_base64( $resource_data['blob'] ) ) {
			$errors[] = __( 'Resource blob content must be valid base64-encoded data', 'meta-mcp' );
		}

		// mimeType is optional. Only its type is checked.
		if ( isset( $resource_data['mimeType'] ) && ! is_string( $resource_data['mimeType'] ) ) {
			$errors[] = __( 'Resource mimeType must be a string if provided', 'meta-mcp' );
		}
