				...
				if ( ! empty( $validation_errors ) ) {
					...
					$this->log_deprecation(
						...
						/* translators: 1: ability name, 2: validation errors */
							__( 'Invalid annotations for resource ability "%1$s" will be dropped: %2$s', 'meta-mcp' ),
		...
		if ( null === $uri ) {
			return new WP_Error(
				...
				/* translators: %s: ability name */
					__( "Resource URI not found in ability meta for '%s'. URI must be provided at 'mcp.uri'.", 'meta-mcp' ),
					$this->ability->get_name()
				)
			);
		}
		...
		if ( ! McpValidator::validate_resource_uri( $uri ) ) {
			return new WP_Error(
				...
				/* translators: 1: ability name, 2: invalid URI */
					__( "Invalid resource URI '%2\$s' for ability '%1\$s'. URI must be RFC 3986 compliant with a scheme.", 'meta-mcp' ),
					$this->ability->get_name(),
					$uri
				)
			);
		...
		if ( ! is_string( $filtered_uri ) || ! McpValidator::validate_resource_uri( $filtered_uri ) ) {
			return new WP_Error(
				...
				/* translators: %s: invalid URI returned by filter */
					__( 'Filter returned invalid MCP resource URI: %s', 'meta-mcp' ),
			...
			if ( $this->validate_type( $value, $type ) ) {
				...
				$this->log_deprecation(
					...
					/* translators: 1: deprecated meta key, 2: new meta key path */
						__( 'Ability meta key "%1$s" is deprecated. Use "mcp.%1$s" instead.', 'meta-mcp' ),
						$key
					),
		...
		try {
			...
			return new WP_Error(
				...
				/* translators: %s: error message */
					__( 'Failed to create Resource DTO for ability %1$s: %2$s', 'meta-mcp' ),
					$ability->get_name(),
					$e->getMessage()
				),
				array( 'exception' => $e )
