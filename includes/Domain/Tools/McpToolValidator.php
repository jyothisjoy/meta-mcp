...
class McpToolValidator {
	...
	public static function validate_tool_data( array $tool_data, string $context = '' ) {
		...
		if ( ! empty( $validation_errors ) ) {
			...
			/* translators: %s: comma-separated list of validation errors */
				__( 'Tool validation failed: %s', 'meta-mcp' ),
	...
	public static function validate_tool_dto( ToolDto $tool ) {
		...
		if ( ! McpValidator::validate_name( $tool->getName() ) ) {
			$errors[] = __( 'Tool name must be 1-128 characters and contain only [A-Za-z0-9_.-]', 'meta-mcp' );
		}
	...
	public static function get_validation_errors( array $tool_data ): array {
		...
		if ( empty( $tool_data['name'] ) || ! is_string( $tool_data['name'] ) || ! McpValidator::validate_name( $tool_data['name'] ) ) {
			$errors[] = __( 'Tool name is required and must only contain letters, numbers, hyphens (-), underscores (_), and dots (.), and be 128 characters or less', 'meta-mcp' );
		}
		...
		if ( isset( $tool_data['description'] ) && ! is_string( $tool_data['description'] ) ) {
			$errors[] = __( 'Tool description must be a string if provided', 'meta-mcp' );
		}
		...
		if ( isset( $tool_data['title'] ) && ! is_string( $tool_data['title'] ) ) {
			$errors[] = __( 'Tool title must be a string if provided', 'meta-mcp' );
		}
		...
		if ( isset( $tool_data['annotations'] ) ) {
			if ( ! is_array( $tool_data['annotations'] ) ) {
				$errors[] = __( 'Tool annotations must be an array if provided', 'meta-mcp' );
			} else {
		...
		if ( isset( $tool_data['_meta'] ) && ! is_array( $tool_data['_meta'] ) ) {
			$errors[] = __( 'Tool _meta must be an object/array if provided', 'meta-mcp' );
		}
	...
	private static function get_schema_validation_errors( $schema, string $field_name ): array {
		...
		if ( ! is_array( $schema ) ) {
			return array(
				...
				/* translators: %s: field name (inputSchema or outputSchema) */
					__( 'Tool %s must be a valid JSON schema object', 'meta-mcp' ),
					$field_name
				),
			);
		}
		...
		if ( ! isset( $schema['type'] ) ) {
			...
			/* translators: %s: field name */
				__( 'Tool %s must specify a root type of \'object\'', 'meta-mcp' ),
				$field_name
			);
		} elseif ( ! is_string( $schema['type'] ) || 'object' !== $schema['type'] ) {
			$errors[] = sprintf(
			/* translators: %s: field name */
				__( 'Tool %s root type must be \'object\'', 'meta-mcp' ),
				$field_name
			);
		}
		...
		if ( isset( $schema['properties'] ) && ! is_array( $schema['properties'] ) ) {
			$errors[] = sprintf(
			/* translators: %s: field name */
				__( 'Tool %s properties must be an object/array', 'meta-mcp' ),
				$field_name
			);
		}
		...
		if ( isset( $schema['required'] ) && ! is_array( $schema['required'] ) ) {
			$errors[] = sprintf(
			/* translators: %s: field name */
				__( 'Tool %s required field must be an array', 'meta-mcp' ),
				$field_name
			);
		}
			...
			foreach ( $schema['properties'] as $property_name => $property ) {
				...
				if ( ! is_array( $property ) ) {
					...
					/* translators: %1$s: field name, %2$s: property name */
						__( 'Tool %1$s property \'%2$s\' must be an object', 'meta-mcp' ),
						$field_name,
						$property_name
					);
					continue;
				...
				$errors[] = sprintf(
				/* translators: %1$s: field name, %2$s: property name */
					__( 'Tool %1$s property \'%2$s\' type must be a string or array of strings (union type)', 'meta-mcp' ),
					$field_name,
					$property_name
				);
			}
		...
		if ( isset( $schema['required'] ) && is_array( $schema['required'] ) ) {
			foreach ( $schema['required'] as $required_field ) {
				if ( ! is_string( $required_field ) ) {
					$errors[] = sprintf(
					/* translators: %s: field name */
						__( 'Tool %s required field names must be strings', 'meta-mcp' ),
						$field_name
					);
					continue;
				}
				...
				/* translators: %1$s: field name, %2$s: required field */
					__( 'Tool %1$s required field \'%2$s\' does not exist in properties', 'meta-mcp' ),
					$field_name,
					$required_field
				);
			}
	...
	private static function get_icons_validation_errors( $icons ): array {
		if ( ! is_array( $icons ) ) {
			return array( __( 'Tool icons must be an array if provided', 'meta-mcp' ) );
		}
	...
	public static function get_execution_validation_errors( $execution ): array {
		if ( ! is_array( $execution ) ) {
			return array( __( 'Tool execution must be an object/array if provided', 'meta-mcp' ) );
		}
		...
		if ( isset( $execution['taskSupport'] ) ) {
			if ( ! is_string( $execution['taskSupport'] ) ) {
				$errors[] = __( 'Tool execution taskSupport must be a string', 'meta-mcp' );
				...
				/* translators: %s: comma-separated list of valid values */
					__( 'Tool execution taskSupport must be one of: %s', 'meta-mcp' ),
			...
			switch ( $field ) {
				...
				case 'openWorldHint':
					if ( ! is_bool( $value ) ) {
						...
						/* translators: %s: annotation field name */
							__( 'Tool annotation field %s must be a boolean', 'meta-mcp' ),
							$field
						);
					}
					break;
				...
				case 'title':
					if ( ! is_string( $value ) ) {
						$errors[] = sprintf(
						/* translators: %s: annotation field name */
							__( 'Tool annotation field %s must be a string', 'meta-mcp' ),
							$field
						);
						break;
					}
					if ( empty( trim( $value ) ) ) {
						$errors[] = sprintf(
						/* translators: %s: annotation field name */
							__( 'Tool annotation field %s must be a non-empty string', 'meta-mcp' ),
							$field
						);
					}
					break;
