 ...
 *
 * Can be bypassed by defining the META_MCP_AUTOLOAD constant to false.
 *
 * @package WP\MCP
 */
...
final class Autoloader {
	...
	public static function autoload(): bool {
		...
		if ( defined( 'META_MCP_AUTOLOAD' ) && false === META_MCP_AUTOLOAD ) {
			return true;
		}
		...
		$autoloader      = META_MCP_DIR . '/vendor/autoload_packages.php';
	...
	private static function missing_autoloader_notice(): void {
		...
		foreach ( $hooks as $hook ) {
			add_action(
				$hook,
				static function () {
					$error_message = __( 'MCP Adapter: The Composer autoloader was not found. If you installed the plugin from the GitHub source code, make sure to run `composer install`.', 'meta-mcp' );
